...
import 'package:rating_summary/rating_summary.dart' as ratingSummary;
import 'package:flutter/widgets.dart' as flutter;
import '../../category/model/sub_product_model.dart';

class CommonProductDetail extends ConsumerStatefulWidget {
  SubProductDetail productDetailData;
  CommonProductDetail({super.key, required this.productDetailData});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _CommonProductDetailState();
}

class _CommonProductDetailState extends ConsumerState<CommonProductDetail> {
  bool isLiked = false;
  ...
  double _validateRating(double? rating) {
    if (rating == null || rating.isNaN || !rating.isFinite) {
      print('Invalid rating value: $rating');
      return 0.0;
    }
    return rating.round().toDouble();
  }
  ...
  int _parseCount(String? count) {
    ...
    if (parsed == null) {
      print('Failed to parse count: $count');
    }
    return parsed ?? 0;
  }
  ...
  Widget build(BuildContext context) {
    ...
    var frameData = widget.productDetailData;
          ...
          appBar: myAppBar(
              ...
              action: [
                ...
                IconButton(
                    onPressed: () {
                      context.go(
                          '/${Routes.bottomNav}/${Routes.commonProductDetail}/${Routes.cart}',
                          extra: frameData);
                    },
                    icon: badges.Badge(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Container(
                                  padding: const EdgeInsets.all(10),
                                  width: double
                                      .infinity, // Changed from 100.w to double.infinity for Flutter compatibility
                                  decoration: BoxDecoration(
                                    borderRadius: BorderRadius.circular(10),
                                    border: Border.all(width: 2),
                                  ),
                                  child: frameData.reviews!.isEmpty
                                      ? Padding(
                                          padding: const EdgeInsets.symmetric(
                                              vertical: 20),
                                          child: Center(
                                      ...
                                      : ratingSummary.RatingSummary(
                                          ...
                                          labelCounterFiveStarsStyle:
                                              Styles.mediumText(context)
                                                  .copyWith(fontSize: 14),
                                          counter:
                                              frameData.reviews?.length ?? 0,
                                          average: _validateRating(
                                              frameData.overallRating),
                                          showAverage: true,
                                          counterFiveStars: _parseCount(
                                              frameData
                                                  .ratingSummary?[4].count),
                                          counterFourStars: _parseCount(
                                              frameData
                                                  .ratingSummary?[3].count),
                                          counterThreeStars: _parseCount(
                                              frameData
                                                  .ratingSummary?[2].count),
                                          counterTwoStars: _parseCount(frameData
                                              .ratingSummary?[1].count),
                                          counterOneStars: _parseCount(frameData
                                              .ratingSummary?[0].count),
                                        ),
                                ),
                                ListView.separated(
                                    separatorBuilder: (context, index) {
